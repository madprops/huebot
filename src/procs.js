...
module.exports = function(Huebot)
{
  ...
  Huebot.send_message = function(ctx, message, feedback=true)
  {
    ...
    Huebot.socket_emit(ctx, 'sendchat', {message:message})	
  }
  ...
  Huebot.send_synth_key = function(ctx, key)
  {
    ...
    Huebot.socket_emit(ctx, "send_synth_key", {key:key})
  }
  ...
  Huebot.send_synth_voice = function(ctx, text)
  {
    ...
    Huebot.socket_emit(ctx, "send_synth_voice", {text:text})
  }
  ...
  Huebot.change_media = function(ctx, args={})
  {
    ...
    if(args.type === "image")
    {
      ...
      Huebot.socket_emit(ctx, 'change_image_source', {src:args.src, comment:args.comment})
    }
    ...
    else if(args.type === "tv")
    {
      ...
      Huebot.socket_emit(ctx, 'change_tv_source', {src:args.src, comment:args.comment})
    }
    ...
    else if(args.type === "radio")
    {
      ...
      Huebot.socket_emit(ctx, 'change_radio_source', {src:args.src, comment:args.comment})
    }
  }
  ...
  Huebot.run_command = function(ctx, cmd, arg, data)
  {
    ...
    {
      Huebot.change_media(ctx, {type:"image", src:command.url, comment:data.comment})
    }

    else if(command.type === "tv")
    {
      Huebot.change_media(ctx, {type:"tv", src:command.url, comment:data.comment})
    }

    else if(command.type === "radio")
    {
      Huebot.change_media(ctx, {type:"radio", src:command.url, comment:data.comment})
    }	
  ...
  Huebot.change_background_mode = function(ctx, data, mode)
  {
    ...
    Huebot.socket_emit(ctx, "change_background_mode", {mode:mode})
  }
  ...
  Huebot.change_theme_mode = function(ctx, data, mode)
  {
    ...
    Huebot.socket_emit(ctx, "change_theme_mode", {mode:mode})
  }
  ...
  Huebot.check_speech = function(ctx)
  {
  ...
  Huebot.selective_play = function(ctx, kind, url)
  {
    if(kind === "image")
    {
      Huebot.change_media(ctx, {type:"image", src:url})
    }

    else if(kind === "tv")
    {
      Huebot.change_media(ctx, {type:"tv", src:url})
    }

    else if(kind === "radio")
    {
      Huebot.change_media(ctx, {type:"radio", src:url})
    }
  }
  ...
  Huebot.get_youtube_stream = function(ctx)
  {
    ...
    .then(res => 
    {
      if(res.items !== undefined && res.items.length > 0)
      {
        ...
        Huebot.change_media(ctx, {type:"tv", src:`https://youtube.com/watch?v=${id}`})
      }
    })
